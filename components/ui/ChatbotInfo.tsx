...
const ChatbotInfo = ({ chatBotId }: { chatBotId: string }) => {
  ...
  >([]);
  const [selectedConvUploadFile, setSelectedConvUploadFile] =
        ...
        <div className="flex mb-6">
          ...
          <label className="w-64 flex justify-between  ml-2 items-center px-2 py-2 text-blue rounded-lg  tracking-wide  border border-blue  cursor-pointer">
            <input
              ...
              className="hidden"
              accept=".txt"
              placeholder={'Upload conv.'}
              onChange={(e) => {
                setSelectedConvUploadFile(e.target.files);
                setSelectedFileType('WHATSAPP');
              }}
              multiple
            />
        ...
        >
          {pdfList?.length > 0 && !showLoader ? (
            ...
            <>
              ...
              />
              <div className="p-8">
                Please upload a Doc to train the AI automatically
              </div>
            </>
          )}
        </div>
